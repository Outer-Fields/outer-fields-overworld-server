 ...
 */
public class GridCombatUtilityTest {
    ...
    public void testTileToGlobal() {
        ...
        int tileSize = WorldSettings.GET().tileSize();
        int chunkSize = WorldSettings.GET().chunkSize().x();  // Assuming square chunks
...
//        var chunksize = GameSettings.GET().chunkSize();
        IAtomicRect2 r1 = IRect2.fromCenterAtomic(IVector2.of(0, 0), WorldSettings.GET().playerViewWithBuffer().add(WorldSettings.GET().playerViewBuffer()));
        IRect2 r2 = IRect2.of(WorldSettings.GET().playerViewBuffer(), WorldSettings.GET().playerViewWithBuffer());
